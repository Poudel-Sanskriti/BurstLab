...
describe("measured results", () => {
  it("counts unique completions and extra attempts", () => {
    const result = metric(
      [job, { ...job, id: "job-002", status: "rejected", attempts: 0 }],
      1000,
      3000,
    );
  ...
  it("replays only observed transitions", () => {
    const run = {
      ...
      jobs: [job],
      phases: { queued: { started_at: 1000, ended_at: 3000 } },
      events: [{ seq: 1, at: 2000, kind: "job", job }],
    } as Run;
    expect(replayAt(run, 0.1).jobs[0].status).toBe("planned");
