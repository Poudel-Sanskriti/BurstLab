...
Install AWS CLI v2, AWS SAM CLI, and Docker for container-based Lambda builds. Configure a local AWS profile using your preferred credential mechanism; keep credentials outside the repository. Choose one region, such as `us-east-1`.

Check current free-plan eligibility, credits, and service availability. Set a small budget alert. Alerts do not stop spending. New accounts can have insufficient concurrency quota for two reserved workers; check Lambda account limits before deployment and request the required quota if needed. Do not silently remove one worker's cap and compare it with a capped worker.
...
Choose stack name **burstlab**, worker concurrency **2**, and **EnableQueue=false**. Review the IAM capabilities and changes before approving the deployment. Both workers should use the same code build, Python runtime, 256 MB memory, and 15-second timeout. The queue visibility timeout is 90 seconds and batch size is one.
...
```bash
.venv/bin/python scripts/aws.py configure --stack burstlab --region us-east-1
.venv/bin/python scripts/aws.py enable --stack burstlab --region us-east-1
.venv/bin/python scripts/aws.py status --stack burstlab --region us-east-1
```
...
```bash
.venv/bin/python scripts/aws.py disable --stack burstlab --region us-east-1
.venv/bin/python scripts/aws.py status --stack burstlab --region us-east-1
```
...
```bash
sam delete --stack-name burstlab --region us-east-1
```
