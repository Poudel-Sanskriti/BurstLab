...
def summarize(run, lane):
    ...
    counts = {
        ...
        for s in ("planned", "waiting", "running", "retrying", *sorted(TERMINAL))
    }
    latencies = [
        max(0, j["finished_at"] - j["submitted_at"])
        for j in jobs
        if j["status"] == "succeeded" and j.get("submitted_at")
    ]
    waits = [
        max(0, j["started_at"] - j["submitted_at"])
        for j in jobs
        if j.get("started_at") and j.get("submitted_at")
    ]
    phase = run.get("phases", {}).get(lane, {})
    elapsed = max(
        0, (phase.get("ended_at") or now_ms()) - phase.get("started_at", now_ms())
    )
    return {
        "counts": counts,
        "total": len(jobs),
        ...
        "unsuccessful": sum(
            counts[s] for s in ("failed", "rejected", "dead_letter", "unresolved")
        ),
        "dispatched": sum(bool(j.get("submitted_at")) for j in jobs),
        "attempts": sum(j.get("attempts", 0) for j in jobs),
        "retries": sum(max(0, j.get("attempts", 0) - 1) for j in jobs),
        "p50_ms": percentile(latencies, 0.5),
        "p95_ms": percentile(latencies, 0.95),
        "wait_p95_ms": percentile(waits, 0.95),
        "latency_samples": len(latencies),
        "elapsed_ms": elapsed,
        "throughput": round(len(latencies) / max(0.001, elapsed / 1000), 2)
        if phase
        else 0,
    }
...
class Engine:
    ...
    def emit(self, run, kind, job=None, **fields):
        ...
        if job is not None:
            event.update(
                job_id=job["id"], lane=job["lane"], job=json.loads(json.dumps(job))
            )
    ...
    async def start(self, config: RunConfig):
        if self.starting or (self.task and not self.task.done()):
        ...
        if self.mode == "aws":
            ...
            try:
                ...
                self.starting = False
        selected = set(
            random.Random(config.seed).sample(
                range(config.count), round(config.count * config.failure_percent / 100)
            )
        )
        ...
        manifest = [
            {
                "id": f"job-{i + 1:03d}",
                "payload": f"{config.payload}#job-{i + 1:03d}",
                "fail_first": i in selected,
            }
        ...
        run = {
            ...
            "notes": [
                ...
                if self.mode == "local"
                else "AWS trial: queue redelivery may take 90 seconds; cross-machine timing has clock uncertainty.",
            ],
        }
    ...
    async def _run(self, run):
        order = [
            run["config"]["first_lane"],
            "queued" if run["config"]["first_lane"] == "direct" else "direct",
        ]
        try:
            for lane in order:
                run["phase"] = lane
            ...
            for job in run["jobs"]:
                if job["status"] not in TERMINAL:
                    self.update(
                        ...
                        job,
                        "unresolved",
                        error="Run ended before a terminal outcome was observed",
                    )
    ...
    async def _local_lane(self, run, lane):
        ...
        async def attempt(job):
            ...
            self.update(
                ...
                job,
                "running",
                started_at=job["started_at"] or start,
            ...
            if injected:
                ...
                self.update(
                    ...
                    job,
                    status,
                    error="Injected first-attempt failure",
                    duration_ms=duration,
                    finished_at=0 if status == "retrying" else now_ms(),
                )
                return False
            try:
                ...
                self.update(
                    ...
                    result_url=f"/api/runs/{run['id']}/artifacts/{lane}/{job['id']}",
                    error="",
                )
                return True
            except Exception as exc:
                self.update(
                    run, job, "failed", finished_at=now_ms(), error=str(exc)[:300]
                )
                return False
        ...
        consumers = (
            [asyncio.create_task(consumer()) for _ in range(config["concurrency"])]
            if lane == "queued"
            else []
        )
        ...
        try:
            for index, job in enumerate(jobs):
                target = began + (
                    index / config["rate"] if config["pattern"] == "steady" else 0
                )
                ...
                job.update(
                    submitted_at=now_ms(),
                    dispatch_lag_ms=max(0, round((time.monotonic() - target) * 1000)),
                )
                if lane == "direct":
                    if active >= config["concurrency"]:
                        self.update(
                            ...
                            "rejected",
                            finished_at=now_ms(),
                            error="Local capacity limit reached (simulated throttle)",
                        )
                    else:
                        active += 1
    ...
    def snapshot(self, run):
        return {
            **run,
            "metrics": {lane: summarize(run, lane) for lane in ("direct", "queued")},
        }
