...
def validate_job(job):
    for key in ("run_id", "id"):
        if not isinstance(job.get(key), str) or not re.fullmatch(r"[a-zA-Z0-9_-]{1,80}", job[key]):
            raise ValueError("Invalid job identity")
    if job.get("lane") not in ("direct", "queued"):
        raise ValueError("Invalid lane")
    if not isinstance(job.get("payload"), str) or not 0 < len(job["payload"].encode()) <= 512:
        raise ValueError("Invalid payload")
    if not isinstance(job.get("delay_ms", 0), int) or not 0 <= job.get("delay_ms", 0) <= 1000:
        raise ValueError("Invalid delay")
    if not isinstance(job.get("max_attempts", 3), int) or not 1 <= job.get("max_attempts", 3) <= 5:
...
def process(job, table=None, s3=None):
    ...
    try:
        claimed = table.update_item(
            ...
            ExpressionAttributeValues={":running": "running", ":owner": owner, ":lease": start + 30000, ":now": start, ":ttl": int(time.time()) + 86400, ":one": 1, ":done": "succeeded", ":dead": "dead_letter"},
    ...
    def event(status, **fields):
        timestamp = now()
        table.put_item(Item={"PK": job["run_id"], "SK": f"EVENT#{timestamp:016d}#{uuid.uuid4().hex}", "type": "event", "job_id": job["id"], "lane": job["lane"], "at": timestamp, "status": status, "attempt": attempt, "expires_at": int(time.time()) + 86400, **fields})

    def finish(status, **fields):
        values = {":owner": owner, ":status": status, ":zero": 0}
        updates = ["#s=:status", "lease_until=:zero"]
        for i, (name, value) in enumerate(fields.items()):
            updates.append(f"{name}=:v{i}")
            values[f":v{i}"] = value
        table.update_item(Key=key, UpdateExpression="SET " + ", ".join(updates), ConditionExpression="owner=:owner", ExpressionAttributeNames={"#s": "status"}, ExpressionAttributeValues=values)
        event(status, **fields)

    event("running", started_at=int(claimed["started_at"]))
    if attempt > job.get("max_attempts", 3):
        finish("dead_letter", finished_at=now(), error="Application attempt limit reached")
    ...
    try:
        ...
        s3.put_object(Bucket=os.environ["BUCKET_NAME"], Key=object_key, Body=artifact.content, ContentType=artifact.media_type, Metadata={"sha256": artifact.checksum})
        elapsed = round((time.monotonic() - began) * 1000)
        finish("succeeded", finished_at=now(), duration_ms=elapsed, object_key=object_key, checksum=artifact.checksum, size_bytes=len(artifact.content), error="")
        return {"status": "succeeded", "object_key": object_key, "duration_ms": elapsed}
    except Exception as exc:
        exhausted = attempt >= job.get("max_attempts", 3)
        status = "failed" if job["lane"] == "direct" else ("dead_letter" if exhausted else "retrying")
        finish(status, finished_at=now() if status != "retrying" else 0, error=str(exc)[:300], duration_ms=round((time.monotonic() - began) * 1000))
        raise
