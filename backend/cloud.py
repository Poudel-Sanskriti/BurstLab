...
class Cloud:
    def __init__(self, config_path: Path):
        ...
        session = boto3.Session(
            region_name=self.region, profile_name=config.get("profile")
        )
        self.lambda_client = session.client(
            "lambda",
            config=Config(
    ...
    def preflight(self, config):
        for name in ("DirectFunction", "QueuedFunction"):
            ...
            cap = self.lambda_client.get_function_concurrency(
                FunctionName=self.outputs[name]
        ...
        mappings = self.lambda_client.list_event_source_mappings(
            ...
            EventSourceArn=self.outputs["QueueArn"],
        )["EventSourceMappings"]
        if not mappings or mappings[0]["State"] != "Enabled":
            raise ValueError(
                "Queue trigger is disabled. Enable it with the AWS helper before running a cloud experiment."
            )
        for queue in ("QueueUrl", "DeadLetterUrl"):
            attributes = self.sqs.get_queue_attributes(
                QueueUrl=self.outputs[queue],
                AttributeNames=[
            ...
            if any(int(v) for v in attributes.values()):
                raise ValueError(
                    "A lab queue still contains work. Drain or clear it before starting a new trial."
                )

    def _items(self, run_id):
        kwargs = {
            "KeyConditionExpression": Key("PK").eq(run_id),
            "ConsistentRead": True,
        }
        items = []
        while True:
    ...
    async def reconcile(self, engine, run):
        ...
        for item in sorted(
            (x for x in items if x.get("type") == "event"),
            key=lambda x: (x["at"], x["SK"]),
        ):
        ...
        for item in (x for x in items if x["SK"].startswith("JOB#")):
            ...
            if fields.get("status") == "succeeded":
                fields["result_url"] = (
                    f"/api/runs/{run['id']}/artifacts/{lane}/{identifier}"
                )
    ...
    async def run_lane(self, engine, run, lane):
                ...
                try:
                    if lane == "queued":
                        await asyncio.to_thread(
                            ...
                            QueueUrl=self.outputs["QueueUrl"],
                        ...
                        response = await asyncio.to_thread(
                            ...
                            FunctionName=self.outputs["DirectFunction"],
                        ...
                        if response.get("FunctionError"):
                            engine.update(
                                ...
                                job,
                                "failed",
                                error="Lambda reported a function error; see attempt details",
                                finished_at=now_ms(),
                            )
                    ...
                    engine.update(
                        ...
                        job,
                        "unresolved",
                        error="Caller outcome unknown: " + str(exc)[:200],
                        finished_at=now_ms(),
                    )
        ...
        async def generate():
            ...
            for i, job in enumerate(jobs):
                target = start + (
                    i / config["rate"] if config["pattern"] == "steady" else 0
                )
                await asyncio.sleep(max(0, target - time.monotonic()))
                job["dispatch_lag_ms"] = max(
                    0, round((time.monotonic() - target) * 1000)
                )
            ...
            for job in jobs:
                if job["status"] not in TERMINAL:
                    engine.update(
                        ...
                        job,
                        "unresolved",
                        error="Observation window ended; check AWS queues before another run",
                    )
        finally:
            if not producer.done():
                producer.cancel()
    ...
    def result_url(self, object_key):
        return self.s3.generate_presigned_url(
            "get_object",
            Params={"Bucket": self.outputs["BucketName"], "Key": object_key},
            ExpiresIn=300,
        )
